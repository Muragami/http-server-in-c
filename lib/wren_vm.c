...
WrenReference* wrenMakeReference(WrenVM* vm, const char* bytes, int len)
{
  WrenReference *ref = ALLOCATE(vm, WrenReference);
  ref->bytes = bytes;
  ref->length = len;
  // hash the contents -> FNV-1a hash. See: http://www.isthe.com/chongo/tech/comp/fnv/
...
void wrenReleaseReference(WrenVM* vm, WrenReference* ref)
{
  // null it out for debugging purposes
  ref->length = 0;
  ref->hash = 0;
  ref->bytes = NULL;
  DEALLOCATE(vm, ref);
...
bool wrenGetSlotBytesEqual(WrenVM* vm, int slot, WrenReference *ref)
{
  validateApiSlot(vm, slot);
  ASSERT(IS_STRING(vm->apiStack[slot]), "Slot must hold a string.");
  ObjString* string = AS_STRING(vm->apiStack[slot]);
  if (string->length != ref->length || string->hash != ref->hash) return false;
  return !memcmp(string->value, ref->bytes, ref->length);
}
...
WrenIterator *wrenNewIterator(WrenVM *vm)
{
  WrenIterator *iter = ALLOCATE(vm, WrenIterator);
  iter->index = 0;
  return iter;
}

void wrenFreeIterator(WrenVM *vm, WrenIterator *iter)
{
  DEALLOCATE(vm, iter);
}

void wrenResetIterator(WrenIterator *iter)
{
  iter->index = 0;
}

bool wrenGetMapNext(WrenVM* vm, int mapSlot, WrenIterator *iter, int keySlot, 
                    int valueSlot)
{
  validateApiSlot(vm, mapSlot);
  validateApiSlot(vm, keySlot);
  ...
  while (IS_UNDEFINED(map->entries[i].key) && i < map->capacity) i++;
  if (i == map->capacity) {
    iter->index = i;
    return false;
  }
...
bool wrenGetListNext(WrenVM* vm, int listSlot, WrenIterator *iter, int valueSlot)
{
  validateApiSlot(vm, listSlot);
  validateApiSlot(vm, valueSlot);
  ASSERT(IS_LIST(vm->apiStack[listSlot]), "Slot must hold a list.");
  ...
  if (iter->index < elements.count) {
    vm->apiStack[valueSlot] = elements.data[iter->index++];
    return true;
  } else {
    return false;
  }
