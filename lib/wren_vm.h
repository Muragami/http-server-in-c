...
// Simple iterator struct for maps and lists, in this impementation
// it is just an integer index into the array or either
typedef struct WrenIterator
{
  uint32_t index;
} WrenIterator;

// A reference creates from a byte string, allowing fast comparison checks
typedef struct WrenReference
{
  uint32_t length;
  uint32_t hash;
  const char *bytes;
