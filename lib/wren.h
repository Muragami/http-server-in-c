...
// An opaque internal type that can iterate a list or map
//
// create/release with wrenNewIterator() and wrenFreeIterator()
typedef struct WrenIterator WrenIterator;

// An opaque internal type that can holds a reference string
//
// using these allows wrenGetSlotBytesEqual() to be nearly as fast as
// internal string comparisons (contains a hash of the contents)
typedef struct WrenReference WrenReference;
...
// create a reference to a string of bytes with length of len
// you must not free the bytes until the reference is released
WREN_API WrenReference* wrenMakeReference(WrenVM* vm, const char* bytes, int len);

// free an allocated reference from wrenMakeReference() above
WREN_API void wrenReleaseReference(WrenVM* vm, WrenReference* ref);
...
// Checks if the string of bytes in slot [slot] equals reference [ref]
// return true on equality and false if not equal.
WREN_API bool wrenGetSlotBytesEqual(WrenVM* vm, int slot, WrenReference *ref);
...
// Create and initialize a WrenIterator for use with
// lists and maps with wrenGetListNext() and wrenGetMapNext()
WREN_API WrenIterator *wrenNewIterator(WrenVM *vm);

// Release the resources of an iterator
WREN_API void wrenFreeIterator(WrenVM *vm, WrenIterator *iter);

// Reinitialize an iterator for use again.
WREN_API void wrenResetIterator(WrenIterator *iter);

// Iterate over all {key: value} pairs in a map, returns true on success
// and false when the iteration is complete. key is place in [keySlot],
// value is placed in [valueSlot]
WREN_API bool wrenGetMapNext(WrenVM* vm, int mapSlot, WrenIterator *iter, int keySlot, 
                    int valueSlot);

// Iterate over all values in a list, return true on success
// and false when the iteration is complete. value is placed in [valueSlot]
WREN_API bool wrenGetListNext(WrenVM* vm, int listSlot, WrenIterator *iter, int valueSlot);
